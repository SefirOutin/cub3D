...
/*   Updated: 2024/03/25 14:16:31 by soutin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_vertical_line_img_to_img(t_img *dst, t_img src, int x, int y, int height)
{
	int	i;
	...
	while (i < height)
	{
		put_pixel_img(dst, x, y + i, get_pixel_img(src, x, i));
		i++;
	}
}
...
void	draw_wall(t_data *data, int x, int ys[2], int ratio, int curr_ray)
{
	...
	ys[0] = half_height;
	ys[1] = half_height;
	wall_height = data->win.h / (data->main.rays[curr_ray].len * 5);
	(ys[0]) += wall_height;
	(ys[1]) -= wall_height;
	start_y = (data->win.h / 2) - (wall_height / 2);
	xxx = (data->main.rays[curr_ray].end.x - (int)data->main.rays[curr_ray].end.x) * 10;
	put_vertical_line_img_to_img(&data->main.view, data->main.textures[0], xxx, start_y, wall_height);
...
void	create_wall(t_data *data,int coef_wall)
{
	...
	while (start.x < 640)
	{
		len_wall = data->win.h / ((data->main.rays[curr_ray].len));
		len_wall *= coef_wall;
		start.y = (data->win.h / 2) - (len_wall / 2);
		create_vertical_line(&data->main.view, start, len_wall, color_wall);
		curr_ray--;
		start.x++;
		curr_ray--;
	}
...
void	create_floor(t_data *data,int coef_wall)
{
	...
	while (start.x < 640)
	{
		len_wall = data->win.h / data->main.rays[curr_ray].len * 3;
		len_wall *= coef_wall;
		start.y = (data->win.h / 2) + (len_wall / 2);
