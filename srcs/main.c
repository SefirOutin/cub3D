...
void	draw_circle(t_data *data, int y_center, int x_center, int r)
{
	...
	while (y >= x)
	{
		mlx_pixel_put(data->mlx_ptr, data->win_ptr, x + x_center, y + y_center,
			0xFF0000);
		mlx_pixel_put(data->mlx_ptr, data->win_ptr, y + x_center, x + y_center,
			0xFF0000);
		mlx_pixel_put(data->mlx_ptr, data->win_ptr, -x + x_center, y + y_center,
			0xFF0000);
		mlx_pixel_put(data->mlx_ptr, data->win_ptr, -y + x_center, x + y_center,
		...
		mlx_pixel_put(data->mlx_ptr, data->win_ptr, y + x_center, -x + y_center,
			0xFF0000);
		mlx_pixel_put(data->mlx_ptr, data->win_ptr, -x + x_center, -y
			+ y_center, 0xFF0000);
		mlx_pixel_put(data->mlx_ptr, data->win_ptr, -y + x_center, -x
			+ y_center, 0xFF0000);
		if (d >= 2 * x)
		{
			d -= -2 * x - 1;
...
void	circle_erase(t_data *data, int centerX, int centerY, int radius)
{
	...
	while (x <= y)
	{
		...
		while (xx <= centerX + y)
		{
			mlx_pixel_put(data->mlx_ptr, data->win_ptr, xx, centerY - x,
				0xFFFFFF);
			mlx_pixel_put(data->mlx_ptr, data->win_ptr, xx, centerY + x,
				0xFFFFFF);
			xx++;
		...
		if (m > 0)
		{
			...
			while (xx <= centerX + x)
			{
				mlx_pixel_put(data->mlx_ptr, data->win_ptr, xx, centerY - y,
					0xFFFFFF);
				mlx_pixel_put(data->mlx_ptr, data->win_ptr, xx, centerY + y,
					0xFFFFFF);
				xx++;
...
int	circle_collision(t_data *data, int x_centre, int y_centre, int r)
{
	...
	while (x > y)
	{
		...
		if (data->map[(y + y_centre + 1) / 50][(x + x_centre + 1) / 50] == 1
			...
			|| data->map[(-y + y_centre - 1) / 50][(x + x_centre + 1) / 50] == 1
			|| data->map[(-y + y_centre - 1) / 50][(-x + x_centre - 1)
			/ 50] == 1)
			return (1);
		if (x != y)
		{
			if (data->map[(x + y_centre + 1) / 50][(y + x_centre + 1) / 50] == 1
				|| data->map[(-x + y_centre - 1) / 50][(y + x_centre + 1)
				/ 50] == 1 || data->map[(x + y_centre + 1) / 50][(-y + x_centre
					- 1) / 50] == 1 || data->map[(-x + y_centre - 1) / 50][(-y
					+ x_centre - 1) / 50] == 1)
				return (1);
		}
	}
...
void	midPointCircleErase(t_data *data, int x_centre, int y_centre, int r)
{
	...
	while (x > y)
	{
		...
		mlx_pixel_put(data->mlx_ptr, data->win_ptr, x + x_centre, y + y_centre,
			0xFFFFFF);
		mlx_pixel_put(data->mlx_ptr, data->win_ptr, -x + x_centre, y + y_centre,
			0xFFFFFF);
		mlx_pixel_put(data->mlx_ptr, data->win_ptr, x + x_centre, -y + y_centre,
			0xFFFFFF);
		mlx_pixel_put(data->mlx_ptr, data->win_ptr, -x + x_centre, -y
			+ y_centre, 0xFFFFFF);
		// If the generated point is on the line x = y then
		// the perimeter points have already been printed
		if (x != y)
		{
			mlx_pixel_put(data->mlx_ptr, data->win_ptr, y + x_centre, x
				+ y_centre, 0xFFFFFF);
			mlx_pixel_put(data->mlx_ptr, data->win_ptr, y + x_centre, -x
				+ y_centre, 0xFFFFFF);
			mlx_pixel_put(data->mlx_ptr, data->win_ptr, -y + x_centre, x
